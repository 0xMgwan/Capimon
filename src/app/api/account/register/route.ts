...
export async function POST(req: Request) {
  ...
  try {
    ...
    const rows = await sql<{ id: string }[]>`
      insert into capx.users (email, password_hash, name, phone, nida_number)
      values (${email}, ${await hashPassword(password)}, ${name}, ${phone}, ${nida})
      returning id`;
    ...
    return NextResponse.json({ ok: true, user: { id: userId, email, name, phone } });
  } catch (e) {
